 ...
 *
 * Return Value:
 * Random point <ARRAY> ([0,0] if invalid zone)
 *
 * Example:
 * [<Area Array>] call aais_waypoint_fnc_areaRandomPos
 *
 * Public: No
 */
...
TRACE_4("",_area,_condition,_checkRadius,_blacklisted);
...
while {_tries < 50} do {
            ...
            if !(_roads isEqualTo []) then {
                {
                    ...
                    if ((_trialPos inArea _area)) exitWith {_found = true};
                } forEach _roads;
            };
        } else {
            _found = true;
    ...
    if (_found) then {
        ...
        if (_checkedPos isEqualTo [] || {_inBlacklisted} || {!(_checkedPos inArea _area)}) then {
            _tries = _tries + 1;
            _found = false;
        } else {
