...
private _sections = QUOTE(!(((str(configname _x)) find QN(PREFIX)) == -1)) configClasses (Configfile >> "Cfg3DEN" >> "Mission");
{
    ...
    //LOG_1("_children: %1",_children);
    {
        ...
        if !(_propertyname isEqualTo "") then {
            ...
            if (_missionvalue isEqualTo "NONE") then {
                if ((missionNamespace getvariable [_propertyname, "NOTSET"]) == "NOTSET") then {
